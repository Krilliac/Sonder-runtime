...
| HTTP chat and A2A turns served by a non-Ollama provider binding | Experimental | Local model steps go through the model gateway; REPL, MCP, autopilot and fleet stay on Ollama, as do HTTP work intents, ensemble and fanout; HTTP web research on a bound tier returns 503 ([observatory-telemetry](docs/architecture/observatory-telemetry.md)). |
