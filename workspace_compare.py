...
def _directory_entries(path):
    ...
    if not before.st_ino or not after.st_ino or _identity(before) != _identity(after):
        raise WorkspaceCompareError("directory identity changed during comparison")
    return names
...
def _inventory(root, authorized, budget):
    ...
    def visit(path, relative):
        ...
        for name in _directory_entries(path):
...
def compare_workspaces(
    ...
    return _fit_output(report, limits["output_bytes"])
