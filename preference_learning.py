...
_UNSAFE_CONTEXT_RE = re.compile(
    ...
    r"[A-Za-z0-9_.-]+\b|"
    r"\b(?:these|those)\s+(?:tests?|files?|functions?|modules?|endpoints?|"
    r"migrations?|queries|scripts?|commands?|runs?|builds?|deploys?)\b|"
    r"\b(?:this|the)\s+(?:test|migration|deploy|build|release|sprint|review|"
    r"refactor|cleanup|run|step|phase|iteration)\b|"
