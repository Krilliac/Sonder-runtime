...
def _stream_sha256(path: Path, max_bytes: int) -> dict:
...
def digest_file(path: str, *, max_bytes: int = DEFAULT_FILE_BYTES, extra_roots: str = "") -> dict:
    ...
    hashed = _stream_sha256(guarded, limit)
    return {
        "algorithm": ALGORITHM,
        "path": str(guarded),
        "max_bytes": limit,
...
def _iter_entries(root: Path, max_depth: int):
    stack = [(root, "", 0)]
    discovered = 0
    while stack:
    ...
    for entry_type, relative, candidate, detail in _iter_entries(
        ...
        try:
            ...
            if size > remaining:
                ...
                break
            hashed = _stream_sha256(guarded, min(remaining, limits["max_file_bytes"]))
