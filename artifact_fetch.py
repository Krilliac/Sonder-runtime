...
def file_sha256(path, chunk=STREAM_CHUNK):
...
def _validate_target(url):
    ...
    """
    return web_tools._validated_public_target(url)
...
def _open_request(url, headers, timeout):
    ...
    return web_tools._urlopen(request, timeout=timeout)
...
def _download(url, part, *, max_bytes, timeout, resume, expect_type, dest_name):
    ...
    for _hop in range(MAX_REDIRECTS + 1):
        headers = {
            "User-Agent": web_tools.USER_AGENT,
            "Accept": "*/*",
