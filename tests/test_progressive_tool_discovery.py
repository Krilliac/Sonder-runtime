...
def test_native_progressive_transport_refuses_hidden_tools_until_schema_load(tmp_path):
    ...
    search = next(row for row in records if row["tool_name"] == "tool_search")
    ...
    assert search["policy_match"] == "native_mcp_discovery"
    assert search["tool_schema_selection"]["visible_names"] == []
    assert loaded["evidence"]["selection_at_start"]["visible_names"] == []
...
def test_native_discovery_refusals_are_audited_without_raw_arguments_or_selection_change(tmp_path):
    from sonder_runtime.adapters.persistence.tool_audit import (
        DurableToolAuditRepository,
    )
    from sonder_runtime.application.ports.tool_executor import ToolResult
    ...
    digest = ToolDiscovery(native_tool_registry()).digest
    private_query = "private-query-" + "x" * 510
    private_tool = "private-schema-name-do-not-persist"
    calls = [
        ("initialize", {"protocolVersion": "2.0", "capabilities": {}}),
        ("tools/call", {"name": "tool_schema", "arguments": {
            "names": ["process_list"], "inventory_digest": digest}}),
        ("tools/call", {"name": "tool_search", "arguments": {"query": private_query}}),
        ("tools/call", {"name": "tool_schema", "arguments": {
            "names": [private_tool], "inventory_digest": digest}}),
        ("tools/call", {"name": "tool_schema", "arguments": {
            "names": ["process_list"], "inventory_digest": digest,
            "forbidden_extra": "private-schema-argument-do-not-persist"}}),
        ("tools/call", {"name": "process_list", "arguments": {}}),
    ]
    ...
    repository = DurableToolAuditRepository(audit_path)
    run_native_mcp(
        SimpleNamespace(config=None, tool_executor=Executor(), tool_audit=repository),
        input_stream=stream, output_stream=output, progressive_tools=True,
    )
    replies = [json.loads(line) for line in output.getvalue().splitlines()]
    selected = json.loads(replies[1]["result"]["output"])["manifest"]["digest"]
    assert "error" in replies[2] and "error" in replies[3] and "error" in replies[4]
    assert replies[5]["result"]["output"] == "process_list:ok"
    assert replies[5]["result"]["evidence"]["tool_schema_selection"]["selection_id"] == selected
    records = repository.read()
    refused_search = next(row for row in records if row["tool_name"] == "tool_search")
    refused_schemas = tuple(row for row in records if row["tool_name"] == "tool_schema" and not row["success"])
    assert refused_search["error_code"] == "tool_search_invalid"
    assert len(refused_schemas) == 2
    assert all(row["terminal"] == "policy_denied" for row in refused_schemas)
    assert all(row["error_code"] == "tool_schema_refused" for row in refused_schemas)
    assert all(row["tool_schema_selection"]["selection_id"] == selected for row in refused_schemas)
    assert all(row["evidence"]["selection_at_start"]["selection_id"] == selected
               for row in refused_schemas)
    assert private_query not in audit_path.read_text()
    assert private_tool not in audit_path.read_text()
    assert "private-schema-argument-do-not-persist" not in audit_path.read_text()
    repository.verify()
...
def test_native_search_does_not_publish_when_durable_audit_fails():
    from sonder_runtime.bootstrap.native_mcp import run_native_mcp
    ...
    run_native_mcp(
        SimpleNamespace(config=None, tool_executor=None, tool_audit=UnavailableAudit()),
        input_stream=input_stream, output_stream=output_stream, progressive_tools=True,
    )
    replies = [json.loads(line) for line in output_stream.getvalue().splitlines()]
    assert "error" in replies[1]
    assert "process_list" not in json.dumps(replies[1])
