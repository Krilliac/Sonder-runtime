...
def app(tmp_path):
    return SimpleNamespace(config=SimpleNamespace(state=SimpleNamespace(workspace_roots=(tmp_path,))),
                           agent_lanes=lambda: 'lane-service')
...
def test_native_catalog_exposes_bounded_agent_control_schema():
    ...
    assert 'parent_session_id' in schema['required']
...
def test_native_lane_calls_preserve_owner_scope_and_parent_binding(monkeypatch, tmp_path):
    captured = []
    def dispatch(service, action, payload, context, parent_session_id, parent_lane_id=None):
    ...
    assert (service, action, payload, session, parent) == ('lane-service', 'spawn', {'task': 'inspect'}, 'parent-1', None)
...
def test_native_lane_denial_never_constructs_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(permission_policy, 'decide_for_caller', lambda *a, **k: SimpleNamespace(action='deny'))
    result = invoke(application, {'action': 'spawn', 'payload': {}, 'parent_session_id': 'parent'})
...
def test_legacy_mcp_exposes_same_lane_service_without_cloud_consent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(permission_policy, 'decide_for_caller', lambda *a, **k: SimpleNamespace(action='allow'))
    result = json.loads(server.agent_lane('list', {}, 'parent'))
    assert result == {'lanes': []}
