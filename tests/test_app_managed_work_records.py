...
from sonder_runtime.application.ports.app_managed_work import WorkSpec, PreparedWorkbenchRun


def spec(**changes):
    return replace(WorkSpec("Inspect and repair the parser", "code", "model:sha256", 12, False), **changes)


def test_spec_digest_binds_exact_task_and_resolved_execution_options():
    original = spec()
    assert original.digest == spec().digest
    for changes in ({"prompt": "Inspect and repair the parser\n"},
                    {"tier": "reasoning"}, {"resolved_model": "other:sha256"},
                    {"max_steps": 13}, {"allow_web": True}):
        assert spec(**changes).digest != original.digest
    assert original.prompt not in repr(original)


@pytest.mark.parametrize("changes", [
    {"prompt": ""}, {"prompt": "x" * 16385}, {"prompt": "a\x00b"},
    {"prompt": "\ud800"}, {"tier": ""}, {"resolved_model": ""},
...
def test_prepared_plan_binds_root_ladder_and_policy(tmp_path):
    ...
    for changes in ({"policy_digest": "b" * 64}, {"allow_location": True},
