"""Shared filesystem locations for trilobite runtime state.
...
def default_home() -> Path:
    override = os.environ.get("TRILOBITE_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    if os.name == "nt":
        root = os.environ.get("LOCALAPPDATA") or os.environ.get("APPDATA")
        if root:
            return Path(root) / "trilobite"
...
def memory_db_path() -> str:
    override = os.environ.get("TRILOBITE_DB", "").strip()
    if override:
        return str(Path(override).expanduser())
