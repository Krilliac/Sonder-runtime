...
class _OperationalCapabilitiesPanel extends StatelessWidget {
  ...
  const _OperationalCapabilitiesPanel({super.key, required this.info, required this.api});
    ...
    return Column(
      ...
      children: [
        ...
        if (info.poolSchemaVersion == 2)
          _StatusRow(label: 'Cached capacity', value: info.poolCapacitySummary, ok: true),
...
class _OllamaPoolDetailsState extends State<OllamaPoolDetails> {
  ...
  Future<void> _load({bool refresh = false, String cursor = ''}) async {
    ...
    setState(() { _busy = true; _error = null; _page = null; });
    try {
      final page = await widget.api.ollamaPoolAdminStatus(refresh: refresh, cursor: cursor);
      if (!mounted || generation != _generation) return;
      setState(() { _page = page; _busy = false; });
    } catch (_) {
      if (!mounted || generation != _generation) return;
      setState(() {
        _busy = false;
        _error = 'Worker details unavailable. Check administrator access, then inspect again.';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Wrap(spacing: 8, children: [
        OutlinedButton(onPressed: _busy ? null : () => _load(), child: const Text('Inspect worker page')),
        OutlinedButton(onPressed: _busy ? null : () => _load(refresh: true), child: const Text('Refresh worker cache')),
        if (page != null && !page.complete && page.nextCursor.isNotEmpty)
          OutlinedButton(onPressed: _busy ? null : () => _load(cursor: page.nextCursor), child: const Text('Next worker page')),
      ]),
      if (_busy) const LinearProgressIndicator(semanticsLabel: 'Loading worker page'),
      if (_error != null) Text(_error!, semanticsLabel: _error),
      if (page != null) ...[
        Text('${page.workers.length} of ${page.workerCount} workers on this page; ${page.omittedWorkerCount} remaining'),
        for (final worker in page.workers)
          SelectableText('${worker.origin}: ${worker.state}; ${worker.modelCount} models; ${worker.errorCategory}\n${worker.modelPreview.join(', ')}'),
      ],
    ]);
  }
}
