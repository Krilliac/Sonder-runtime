...
void main() {
  testWidgets('worker detail stays idle until requested and clears after denial', (tester) async {
    var calls = 0;
    final client = MockClient((request) async {
      calls++;
      if (calls == 2) return http.Response('{}', 403);
      expect(jsonDecode(request.body)['refresh'], isFalse);
      return http.Response(jsonEncode({'schema_version': 2, 'worker_count': 64,
        'complete': false, 'next_cursor': 'opaque', 'omitted_worker_count': 63,
        'workers': [{'origin': 'https://private-worker.test:11434', 'state': 'ready',
          'model_count': 1, 'model_preview': ['safe'], 'error_category': 'none'}]}), 200);
    });
    await http.runWithClient(() async {
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: OllamaPoolDetails(
