...
class _LazyService:
    """Keep optional REPL providers behind the interface compatibility seam."""
    ...
    def __getattr__(self, name: str):
        provider = importlib.import_module(self._module_name)
        if self._attribute is not None:
            provider = getattr(provider, self._attribute)
        return getattr(provider, name)


# The REPL is an optional interface boundary. Lazy providers preserve its
# historical failure behavior without making this packaged module depend on
# the adapter layer at import time.
command_catalog = _LazyService("sonder_runtime.adapters.command_catalog")
intents = _LazyService("sonder_runtime.adapters.content_services", "intents")
...
_RULES = [
    ...
    _rule(r"^(?:run|use)\s+(?:a\s+)?local\s+workbench\s+agent\s+"
          ...
          _with_arg("/work")),
    _rule(r"^work\s+on\s+(?:this\s+)?task\s*[:,]?\s*(?P<arg>.+)$",
          _with_arg("/work")),

    # --- weather ---
    _rule(r"^(?:check|show|get)\s+(?:the\s+)?weather\s+(?:for|in)\s+"
          r"(?P<arg>.+?)\s*[?!.]*$", _with_arg("/weather")),
    ...
    _rule(r"^(?:verify|check)\s+(?:the\s+)?generated\s+(?:artifact|files?)\s+"
