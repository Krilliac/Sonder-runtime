...
class TerminalOutputReference:
    ...
    def __post_init__(self):
        ...
        if type(self.size_bytes) is not int or not 0 <= self.size_bytes <= MAX_OUTPUT_BYTES:
...
class TerminalOutputStore(Protocol):
    ...
    def put(self, binding: ProjectionBinding, output: str, *, context: OperationContext) -> TerminalOutputReference:
        """Commit immutable exact UTF-8 output before returning its scoped reference."""
        ...

    def get(self, binding: ProjectionBinding, reference: TerminalOutputReference, *, context: OperationContext) -> str:
