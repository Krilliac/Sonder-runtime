...
def run(root, namespace, job_id):
    ...
    with PrivateDirectoryAnchor(root) as anchor, PrivateDirectoryAnchor(workspace) as workspace_anchor:
        anchor.validate()
...
def _run(root, workspace, namespace, job_id, anchor):
    from ..adapters.persistence.owned_sqlite import OwnedSQLiteConnections, install_disposable_owner
    ...
    from .managed_configuration import read_configuration, COMPONENTS, CLOSE_ORDER, MANIFEST_DIGEST
    journal = SQLiteManagedRuntimeOwnerJournal(root / "owner.sqlite", namespace=namespace)
    pending = journal.pending()
    record = SQLiteDurableJobRegistry(root / "processes.sqlite").view(job_id)
    identity = process_identity(os.getpid())
    status = journal.status()
    if pending is None or pending.action != "launch" or pending.operation_id != job_id or pending.namespace != namespace or pending.incarnation != status["incarnation"] or record.process_id != os.getpid() or not identity or dict(record.metadata).get("process_instance_identity") != identity:
    ...
    config = replace(config,
        server=replace(config.server, host="127.0.0.1", port=descriptor["port"], auth_mode="api-key",
        ...
        state=replace(config.state, home=str(root / "state"), workspace_roots=(str(workspace),)),
        ollama=replace(config.ollama, url=f"http://127.0.0.1:{descriptor['port']}"),
        secrets=Secrets(api_key=secrets.token_hex(32)), private_source_paths=(str(root),),
    ...
    os.environ.update({"SONDER_HOST": "127.0.0.1", "SONDER_PORT": str(descriptor["port"]),
        "SONDER_AUTH_MODE": "api-key", "SONDER_API_KEY": config.secrets.api_key,
        "OLLAMA_HOST": config.ollama.url, "SONDER_OLLAMA_WORKERS": "", "SONDER_ALLOW_REMOTE_OLLAMA": "0"})
    ...
    def close_sqlite(resource, timeout):
        ...
        return proof("sqlite", cleanup() and resource.snapshot().clean, "exact-sqlite-handles")
    resources.initialize("sqlite", lambda: sqlite, close_sqlite)
    resources.initialize("workers", lambda: workers,
        lambda resource, timeout: proof("workers", resource.close(timeout=timeout).clean, "exact-worker-handles"))
    from ..adapters.persistence import migrations
    migrations.migrate_all(busy_timeout_ms=1000)
    from .app import build_application, install_owned_application, stop_owned_application
    ...
    def construct():
        application = build_application(config=config, child_repository_factory=HostChildRepositoryFactory(
            "sqlite", lambda: SQLiteDurableContinuationRepository(Path(descriptor["child_path"]))))
    ...
    watcher = workers.thread(target=control, name="managed-runtime-control", daemon=True)
    watcher.start()
    try:
        serve.main(config=config, _server_factory=factory, _close_default_resources=False)
    finally:
        stopped.set()
...
if __name__ == "__main__":
    try:
        ...
        location = ";".join(Path(frame.filename).name + ":" + str(frame.lineno) for frame in frames[-3:])
        print("managed runtime failed: " + type(error).__name__ + " " + location, file=sys.stderr)
        raise SystemExit(1)
