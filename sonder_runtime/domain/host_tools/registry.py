...
HOST_TOOL_SPECS: tuple[ToolSpec, ...] = (
    ...
    # Crash and profile digest engines (bootstrap/debug_tools.py). The
    # Windows Performance Toolkit and SDK extras are inventory-only.
    _spec("llvm-symbolizer", C.DEBUGGER_PROFILER,
          executables=("llvm-symbolizer", "llvm-symbolizer-20", "llvm-symbolizer-19",
                       "llvm-symbolizer-18")),
    _spec("llvm-pdbutil", C.DEBUGGER_PROFILER),
    _spec("addr2line", C.DEBUGGER_PROFILER, platforms=UNIX),
    _spec("eu-stack", C.DEBUGGER_PROFILER, platforms=LINUX),
    _spec("heaptrack_print", C.DEBUGGER_PROFILER, ("-v",), platforms=LINUX),
    _spec("callgrind_annotate", C.DEBUGGER_PROFILER, None, platforms=UNIX),
    _spec("minidump-stackwalk", C.DEBUGGER_PROFILER,
          executables=("minidump-stackwalk", "minidump_stackwalk")),
    _spec("dump_syms", C.DEBUGGER_PROFILER),
    _spec("tracy-csvexport", C.DEBUGGER_PROFILER, None),
    _spec("unshare", C.DEBUGGER_PROFILER, platforms=LINUX),
    _spec("symchk", C.DEBUGGER_PROFILER, None, platforms=WIN),
    _spec("wpr", C.DEBUGGER_PROFILER, None, platforms=WIN),
    _spec("procdump", C.DEBUGGER_PROFILER, None, platforms=WIN),
    # -- package managers ---------------------------------------------------
