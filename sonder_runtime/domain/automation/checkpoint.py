"""Agent session checkpoint and replay.

Allows long-running agent sessions to persist intermediate state so they
can resume after interruption (crash, timeout, manual pause) without
replaying the entire history from scratch.

Checkpoints are lightweight JSON snapshots stored alongside the session's
working database.  Each checkpoint captures the minimum state needed to
resume: current step, accumulated context, and any pending decisions.

Thread-safe: concurrent readers and a single writer per session.
"""
