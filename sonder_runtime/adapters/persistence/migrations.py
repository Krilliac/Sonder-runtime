...
from sonder_runtime.platform import version as platform_version

# Migration files remain a deliberately immutable repository-level asset: the
# historical baseline modules are imported by compatibility tests and are
# shipped by the package manifest.  The runtime registry itself lives in the
...
def migrate_store(
    ...
    *,
    ...
    with migration_lock():
        ...
        version = platform_version.build_info().version
