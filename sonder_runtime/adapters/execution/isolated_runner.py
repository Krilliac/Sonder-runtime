...
def build_runtime_argv(
    ...
    *,
    ...
    # Build outputs must never fill a host bind; only bounded tmpfs is allowed.
    if build_scratch_mb is not None and (
        type(build_scratch_mb) is not int or not 64 <= build_scratch_mb <= memory_mb
    ):
        raise ValueError("build scratch must be an integer between 64 MiB and container RAM")
    ...
    argv = [
        ...
        *([f"--tmpfs=/build:rw,nosuid,nodev,mode=1777,size={build_scratch_mb}m"]
          if build_scratch_mb is not None else []),
        ...
        "HOME=/build/home" if build_scratch_mb is not None else "HOME=/tmp",
        "TMPDIR=/build/tmp" if build_scratch_mb is not None else "TMPDIR=/tmp",
        "LANG=C.UTF-8", "--",
    ]
    argv.extend(command)
...
def _run_bounded(
    ...
    timeout, output_limit, project, expected_identity, *, verify_exit_cleanup=False,
):
...
def run_isolated(
    ...
    *,
    ...
    argv, name = build_runtime_argv(
        ...
        build_scratch_mb=build_scratch_mb,
    )
    ...
    result = _run_bounded(
        ...
        **({"verify_exit_cleanup": True} if verify_exit_cleanup else {}),
    )
    result.update({
