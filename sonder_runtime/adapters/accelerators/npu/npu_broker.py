...
class NpuBroker:
    ...
    def _spawn_worker(self, manifests=None):
        ...
        packaged_worker = (
            self._worker_path.name == "npu_worker.py"
            and self._worker_path.parent.name == "npu"
            and self._worker_path.parent.parent.name == "accelerators"
        )
        if packaged_worker:
            command = [
                sys.executable, "-X", "utf8", "-m",
                "sonder_runtime.adapters.accelerators.npu.npu_worker",
            ]
            worker_cwd = self._worker_path.parents[4]
        else:
            command = [sys.executable, "-X", "utf8", str(self._worker_path)]
            worker_cwd = self._worker_path.parent
        creationflags = 0
        if sys.platform == "win32":
            creationflags = (
        ...
        try:
            proc = subprocess.Popen(
                ...
                # The worker now lives inside the package tree. Run it from
                # the distribution root so its package imports resolve in a
                # standalone child process just as they did for the retired
                # root worker.
                cwd=str(worker_cwd),
