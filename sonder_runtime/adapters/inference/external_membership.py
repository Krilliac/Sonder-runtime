...
class _PinnedTransport:
    ...
    def retrieve(self, policy, *, path, timeout, max_bytes, method="GET", data=None):
        ...
        try:
            ...
            context.check_hostname = False  # exact SAN check below; never CN/wildcard fallback
            context.verify_mode = ssl.CERT_REQUIRED
            context.minimum_version = ssl.TLSVersion.TLSv1_2
            context.load_verify_locations(cafile=self._config.trust_anchor_file)
            context.load_cert_chain(certfile=self._secrets.membership_client_cert_file,
                                    keyfile=self._secrets.membership_client_key_file)
...
class ExternalMembershipSource:
    ...
    def read_snapshot(self, *, limits):
        try:
            ...
            if type(raw) is not bytes or len(raw) > maximum:
                raise ValueError
            with Path(self._config.signature_public_key_file).open("rb") as stream:
                encoded_key = stream.read(16385)
            if len(encoded_key) > 16384:
                raise ValueError
            key = serialization.load_pem_public_key(encoded_key)
    ...
    def open_worker_url(self, request, *, timeout):
        ...
        except (MembershipSourceError, ModelCallError):
            raise
        except Exception:
            raise MembershipSourceError("external membership unavailable") from None
