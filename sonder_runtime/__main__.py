...
def _export_runtime_environment(config, *, include_typed_runtime: bool = True) -> None:
    ...
    # Ollama is still constructed by the legacy server boundary after the
    # typed serve configuration is bound.  These two compatibility variables
    # therefore must cross the canonical serve path as well; otherwise TOML
    # workers are validated and then silently discarded before server import.
