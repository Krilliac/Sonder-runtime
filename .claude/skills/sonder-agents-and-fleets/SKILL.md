...
| Worker cap not honored | Only one unquoted, affirmative "use N workers" phrase at the start of the request parses; quotes/backticks, negation, comparatives, a second count, or explanatory/quoting words (`ignore`, `quote`, `phrase`, `document`, `instruction`, `example`, `say(s)`, `mention(s)`, `explain`, `why`) disable inference (`master_orchestrator.requested_worker_cap`). The route header / console prints `note:` with the disabling word (`worker_request_ignored_reason`) - pass `worker_cap=` or use `/master fleet <task>` |
| Fleet "stalled; result is uncertain" on a slow host | The progress deadline defaults to `max(120, SONDER_TIMEOUT + 60)`; a lane inside a model call is never stalled before that call's timeout + 60 s even with a shorter explicit `SONDER_FLEET_PROGRESS_DEADLINE_SECONDS` (`fleet_store.lane_progress_deadline`) |
| Autopilot paused on "wall-clock budget" | Per-invocation wall budget (`SONDER_AUTOPILOT_MAX_WALL_SECONDS`, default 3600 s) alongside `max_cycles`; review, then `/autopilot resume <id>` |
| `/autopilot steer` refused in the console | Only runs started from the console after the owner fix carry the console owner (`rc-...`); older unowned runs cannot be steered - cancel and restart them |
