# Sonder Runtime — agent guidance (Codex)

Append to `~/.codex/AGENTS.md` once the MCP server is registered per
[config.toml.example](config.toml.example). The guidance below is identical to
the Claude variant — it describes the model's behaviour, not the harness.
