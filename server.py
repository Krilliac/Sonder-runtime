...
def improvement_report_data(session: str = "", project: str = "") -> dict:
    ...
    gated_positive, acceptance_basis = learning_health.gating_positive_percent(
        learning_state
    )
    acceptance = (
    ...
    if acceptance_basis == "reviewed" and gated_positive < 60.0:
        add(
            "learning",
            "medium",
            "Caller-judged work succeeds %.1f%% of the time (%d reviewed outcome(s))."
            % (gated_positive, reviewed),
    ...
    elif acceptance_basis == "unmeasured" and outcomes >= 200:
    ...
    if outcomes >= 200 and acceptance_basis == "unmeasured":
        score = min(score, 75)
    return {
        "score": score,
...
def tool_manifest() -> str:
    ...
    tools = {
        ...
        "model_fanout/model_fanout_status/model_fanout_cancel/model_fanout_resume": "Run a durable, bounded fanout across discovered local, cloud, or all chat models; inspect its owner-scoped receipt, cancel it, or explicitly retry finished results. Natural chat supports `ask all available local models: ...`, `ask all local and cloud models: ...`, `ask all local models and cloud models: ...`, `run every available cloud models to answer: ...`, `ask the phi4:latest model to ...`, and `run using model phi4:latest: ...`. Cloud use still needs explicit operator opt-in; shared deployments restrict fanout to developer-authorized callers.",
