...
| `GET /v1/client/schema` | any authorized caller | API-008 client/SDK schema envelope (`{"type":"client_schema","version":1,"schema":{...,"digest"}}`) derived from the served typed tool catalog; clients cache it and advertise its `digest` on reconnect. |
| `POST /v1/client/reconnect` | any authorized caller | API-007 reconnect plan for a `{"type":"reconnect","version":1,...}` body: a stale or malformed digest returns `refresh_schema`, a known stream resumes from its watermark in batches of at most `batch_limit` with `has_more`, and an unknown stream or out-of-range watermark is `rejected`. The only stream is the in-memory `control` stream of `control.snapshot` permission-mode events; it restarts at sequence 1 with the process. A malformed body is a 400. |
