# Sonder Memory + Reward Loop — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend the `sonder-runtime` MCP server so local coding traffic is memory-augmented, captured, scored by real outcomes, and distilled into retrievable lessons — a self-improving runtime loop around a frozen Ollama model, exposed through the `sonder` tool and route.

**Architecture:** Flat sibling modules in `~/.claude/mcp-servers/sonder-runtime/`, imported by `server.py`. Pure logic (store, retrieval, reward, reflection, orchestration) is isolated behind injectable functions so it is unit-testable without a live Ollama. `server.py` wires the Ollama HTTP calls into that logic and exposes three tool surfaces: the wrapped `offload` (learns transparently), a new `sonder` tool (interactive front door), and `record_outcome` (feeds the reward signal back).
...
- **Repo root:** `~/Sonder-runtime` — all paths below are relative to it. All commands run with cwd = repo root.
...
- **Names:** Sonder Runtime is the orchestration product; `sonder` is its tool and route ID. The separate Ollama alias is `sonder:latest` (`FROM qwen2.5-coder:7b`), a model-store entry rather than the runtime itself; the tool can fall back to the raw `code` tier model if that alias is absent.
...
git commit -m "chore: add pytest harness and dev deps for sonder loop"
```
...
Wire the pure logic into Ollama HTTP calls; add `learn` to `offload`, add `sonder` and `record_outcome` tools, footer helpers, thread-safe DB. Unit-test the pure helpers (footer, id, prompt); the tool bodies themselves are exercised by the manual smoke test in Task 9.
...
- Produces (module-level, importable for tests):
  ...
  - `resolve_sonder_model() -> str` (returns `"sonder"` if the alias exists in `/api/tags`, else `TIERS["code"]`)
  - tools: `offload(..., learn=True)`, `sonder(...)`, `record_outcome(interaction_id, signal)`
...
- [ ] **Step 5: Edit `server.py` — add the `sonder` and `record_outcome` tools immediately after `offload`.**
...
def sonder(
    ...
    """Ask through Sonder Runtime's local learning route.
    ...
    | "compiled" | "rejected" | "failed") so the runtime's grounded memory
    improves over time. Defaults to the 7B coder / `sonder:latest` Ollama alias.
    """
    if tier == "code":
        model = resolve_sonder_model()
    else:
        model = TIERS.get(tier, resolve_sonder_model())
    gen = _make_generate(model, system, temperature, num_predict, num_ctx)
    try:
        with _LOCK:
            response, iid = orchestrator.run_with_learning(
                _db(), prompt, "sonder", gen
            )
    except urllib.error.URLError as e:
        return ("ERROR contacting Ollama at %s: %s. Is the Ollama server "
                "running? (`ollama serve`)" % (BASE, e))
...
git commit -m "feat: wire sonder learning loop into MCP server (offload learn, sonder, record_outcome)"
```
...
Create the stable `sonder:latest` rollback alias, document the runtime/alias boundary, and verify the loop end-to-end against the real Ollama.
...
- Produces: `setup_alias.py` with `main()`; the `sonder:latest` Ollama alias/model-store entry.

- [ ] **Step 1: Create `setup_alias.py`**

```python
"""One-time setup: pull the embed model and create the 'sonder' Ollama alias.

The alias is a stable rollback entry (FROM the 7B coder), not the Sonder Runtime
identity. Sub-project #3 publishes validated personal artifacts separately as
`sonder-personal:latest`; it does not overwrite the rollback alias.
...
MODELFILE = (
    ...
    'SYSTEM "You are the local language model operating inside Sonder Runtime. '
    'Sonder Runtime is the orchestration host, not a model or set of weights. '
...
def main():
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            f.write(MODELFILE)
        print("Creating 'sonder' alias ...")
        subprocess.run(["ollama", "create", "sonder", "-f", path], check=False)
    finally:
        os.unlink(path)
    print("Done. Verify with: ollama list | findstr sonder")
...
```
Expected: pulls embed model, prints `Creating 'sonder' alias ...`, then `Done.`

- [ ] **Step 3: Verify the alias exists**

Run:
```bash
ollama list | grep sonder
```
Expected: a `sonder:latest` row.
...
server._DB_PATH = os.path.join(tempfile.gettempdir(), 'sonder_smoke.db')
r = server.sonder('Write a Python one-liner that reverses a string.')
print('RESP:', r[:200])
...
"
```
Expected: a real code response ending in an `[interaction_id: ...]` footer; first `record_outcome` prints `Recorded 'tests_passed' (reward +1.00) ... Distilled lesson <id>.`; the loop ran end-to-end against Ollama. (Delete the temp DB afterward: `rm "$TEMP/sonder_smoke.db"`.)

- [ ] **Step 5: Update `README.md` — append a "Sonder self-learning loop" section**

Add at the end of `README.md`:

```markdown
## Sonder Runtime — self-learning orchestration loop

Sonder Runtime is a self-improving orchestration loop around selected inference
models. Runtime learning lives in this MCP server (retrieval + reflection +
reward); Ollama serves the selected frozen weights. The `sonder` name in Ollama
is a stable alias/model-store entry, not the runtime itself.

- **Interactive use:** call `mcp__sonder-runtime__sonder("...")` — or just say
  "use sonder". The reply ends with `[interaction_id: <id>]`.
...
- **Setup (one-time):** `./venv/Scripts/python.exe setup_alias.py` (pulls
  `nomic-embed-text`, creates the `sonder` Ollama alias).
...
git commit -m "feat: sonder Ollama alias, setup script, and docs"
```
...
- `sonder` general-session runtime tool + distinct Ollama rollback alias + fallback → Tasks 8, 9. ✓
