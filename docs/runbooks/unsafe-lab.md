...
approval token. That shared approval bypass reaches every direct MCP call path
that consults it, so direct MCP callers are inside the unsafe-mode blast radius
too. Hosted agents still cannot invoke local-only workspace, artifact-risk, or
process-risk tools; unsafe mode bypasses only their nested-model restriction. The configured
