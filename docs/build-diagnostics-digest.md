...
| HTTP `POST /v1/tools/output-digest` | Takes exactly one of `job_id` or `path`, plus `tail_lines` and `max_failure_lines`. It is one typed `output_digest` call as the authenticated principal, with developer or admin authority. Job digests are principal-scoped (not operator) and a foreign job is `404 JOB_NOT_FOUND`. A path outside the guarded surface is `403 DIGEST_SOURCE_REJECTED`. See [structured test runs over HTTP](structured-test-runs.md#over-http). |
| App chat `/digest <job_id\|path>`, `/test ...` | The HTTP chat dispatcher makes the same typed calls as the HTTP routes, as the authenticated principal. It is not operator-scoped: a job-shaped argument is looked up as the caller's own job first, then as a file of that name. See [structured test runs in the app chat](structured-test-runs.md#in-the-app-chat). |
| HTTP `POST /v1/tools/test-run`, `GET /v1/tools/test-run/{id}` | Start a structured test run and read its report through the typed gateway. The status is `202` while the run is going and `200` with the report. |
