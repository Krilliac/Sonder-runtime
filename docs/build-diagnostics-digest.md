...
| HTTP `POST /v1/tools/output-digest` | Takes exactly one of `job_id` or `path`, plus `tail_lines` and `max_failure_lines`. It is one typed `output_digest` call as the authenticated principal, with admin authority. Job digests are principal-scoped (not operator) and a foreign job is `404 JOB_NOT_FOUND`. A path outside the guarded surface is `403 DIGEST_SOURCE_REJECTED`. See [structured test runs over HTTP](structured-test-runs.md#over-http). |
