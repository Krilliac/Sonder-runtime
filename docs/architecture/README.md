...
- WP1 Three-Hundred-Thirty-Fifth Slice: `WP1-THREE-HUNDRED-THIRTY-FIFTH-SLICE.md` — selfmod test command construction now lives in sonder_runtime.domain.automation.selfmod_test_commands, preserving the root _selfmod_test_commands compatibility alias
- WP1 Three-Hundred-Thirty-Sixth Slice: `WP1-THREE-HUNDRED-THIRTY-SIXTH-SLICE.md` — approval listing limit parsing now lives in sonder_runtime.domain.approvals_limit, preserving the root _approvals_limit compatibility alias
- WP1 Three-Hundred-Thirty-Seventh Slice: `WP1-THREE-HUNDRED-THIRTY-SEVENTH-SLICE.md` — callable keyword inspection now lives in sonder_runtime.domain.callable_inspection, preserving the root _callable_accepts_keyword compatibility alias
- WP1 Three-Hundred-Thirty-Eighth Slice: `WP1-THREE-HUNDRED-THIRTY-EIGHTH-SLICE.md` — fanout worker identity now lives in sonder_runtime.domain.fanout_worker_identity, preserving the root _FANOUT_WORKER_INSTANCE alias and _fanout_worker_id compatibility delegate
- WP1 Three-Hundred-Thirty-Ninth Slice: `WP1-THREE-HUNDRED-THIRTY-NINTH-SLICE.md` — runtime policy update JSON parsing now lives in sonder_runtime.domain.runtime_update_parsing, preserving the root _runtime_update_object alias
- WP1 Three-Hundred-Fortieth Slice: `WP1-THREE-HUNDRED-FORTIETH-SLICE.md` — execution-decision summary header formatting now lives in sonder_runtime.domain.execution_route_formatting, preserving the root _execution_route_header compatibility delegate
- WP1 Three-Hundred-Forty-First Slice: `WP1-THREE-HUNDRED-FORTY-FIRST-SLICE.md` — agent tool observation quality classification now lives in sonder_runtime.domain.agent_observation_quality, preserving the root _agent_observation_ok alias
- WP1 Three-Hundred-Forty-Second Slice: `WP1-THREE-HUNDRED-FORTY-SECOND-SLICE.md` — agent run-created-paths key normalization now lives in sonder_runtime.domain.agent_path_keys, preserving the root _agent_created_path_key alias
- WP1 Three-Hundred-Forty-Third Slice: `WP1-THREE-HUNDRED-FORTY-THIRD-SLICE.md` — agent model-escalation identity key now lives in sonder_runtime.domain.agent_escalation_identity, preserving the root _agent_escalation_key alias
- WP1 Three-Hundred-Forty-Fourth Slice: `WP1-THREE-HUNDRED-FORTY-FOURTH-SLICE.md` — cloud tier classification now lives in sonder_runtime.domain.model_routing.is_cloud_tier, preserving the root _is_cloud_tier compatibility delegate
- WP1 Three-Hundred-Forty-Fifth Slice: `WP1-THREE-HUNDRED-FORTY-FIFTH-SLICE.md` — fanout worker id construction now delegates to sonder_runtime.domain.fanout_worker_identity.fanout_worker_id, removing duplicated logic from root
- WP1 Three-Hundred-Forty-Sixth Slice: `WP1-THREE-HUNDRED-FORTY-SIXTH-SLICE.md` — agent help text tool-name parsing now lives in sonder_runtime.domain.agent_help_parsing, preserving the root _agent_help_advertised_tools alias
- WP1 Three-Hundred-Forty-Seventh Slice: `WP1-THREE-HUNDRED-FORTY-SEVENTH-SLICE.md` — agent mutation record convenience wrapper now lives in sonder_runtime.adapters.agent_work_coverage.mutation_record, preserving the root _agent_mutation_record alias
- WP1 Three-Hundred-Forty-Eighth Slice: `WP1-THREE-HUNDRED-FORTY-EIGHTH-SLICE.md` — loop result formatting now lives in sonder_runtime.domain.loop_result_formatting, preserving the root _loop_text_result alias
- WP1 Three-Hundred-Forty-Ninth Slice: `WP1-THREE-HUNDRED-FORTY-NINTH-SLICE.md` — identifier resolution now lives in sonder_runtime.domain.identifier_resolution, preserving the root _resolve_session and _resolve_project delegates
- WP1 Three-Hundred-Fiftieth Slice: `WP1-THREE-HUNDRED-FIFTIETH-SLICE.md` — cloud agent tool policy now lives in sonder_runtime.domain.cloud_agent_tool_policy, preserving the root _cloud_agent_tool_policy_error delegate
- WP1 Three-Hundred-Fifty-First Slice: `WP1-THREE-HUNDRED-FIFTY-FIRST-SLICE.md` — schema coverage annotation now lives in sonder_runtime.domain.schema_policy.with_schema_coverage, preserving the root _with_schema_coverage alias
