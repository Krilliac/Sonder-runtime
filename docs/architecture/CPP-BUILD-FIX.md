...
| App chat `/build`, `/fix-build` | Implemented | The HTTP chat dispatcher parses a line with the REPL facade's grammar and makes the same typed gateway call as `/v1/build/*`, as the caller, unattended. Developer authority is required. The chat's permission gate narrows `/build model` and `status` to their safe members, as the console does. A refused launch in chat names no call, so approve it by `call_id` through the HTTP route. |
...
- `server.py` (server lane):
  ...
  - done: `build_job` and `build_job_result` are `grounded_outcomes`
    ...
    (`ToolApplicationFacade.add_receipt_observer`) on every graph bound to the
    legacy runtime: the one `server._application()` builds itself, and the one
    an entrypoint (`repl`, `serve`, `mcp`) composes with `default_app` and
    hands over through `legacy_root.configure_application`. A REPL `/build`
    report or an HTTP build-route report therefore judges a pending
    generation of the same project
    (`tests/test_grounded_outcomes_typed_build.py`). No legacy tool alias
    reaches `build_job` (`typed_tools.py` maps it to no legacy name). The verdict
