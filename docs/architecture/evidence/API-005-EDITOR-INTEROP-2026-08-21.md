...
and negotiates the intersection of client/server capability identifiers. It
also represents per-request cancellation as a versioned envelope. The thin
...
delivers cancellation through an application callback, exposes the negotiated
capability set, and redacts handler exception details. Filesystem and tool
operations remain application-owned.
...
initialization, capability negotiation, cancellation, correlation, and
error-containment slices. A separately deployed editor transport and live SDK
client remain outside this evidence; the HTTP/MCP surfaces must continue
routing imported rules through their existing instruction and policy
boundaries. ACP elicitation and cross-implementation interoperability remain
adapter-level work.
