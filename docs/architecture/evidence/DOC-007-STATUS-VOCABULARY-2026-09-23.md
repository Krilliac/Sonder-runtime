...
remains review work. The unsafe-lab count of "46 direct MCP call paths" in
`SECURITY.md` is pinned only by a text assertion in `tests/test_unsafe_lab.py`.
A static scan of `server.py` did not reproduce that number, and it is left as
a follow-up.
