...
This slice implements private recovery service composition and runtime-owned slot prerequisites for explicit HTTP preparation, attachment, verification resume, status and close. **Production startup does not yet activate these services.** The fixed disposable `ManagedRuntimeOwner` profile leaves both app control and host control disabled and intentionally seals an empty work slot. Normal HTTP startup can configure app-control metadata, but it does not install an owned work dispatcher; enabling metadata alone does not make `/v1/app-control/work` or recovery execution available. An explicitly authorized production configuration and owned service assembly remain incomplete roadmap work. Attachment and verifier approval remain separate host-controlled calls. HTTP does not grant approval, and original prepared work and terminal receipts remain immutable.
...
- Fourteen registry, real loopback HTTP and actual isolated owned-slot checks passed. The HTTP fixture explicitly supplies private service composition; the owned-slot checks run separately. Neither proves activation through the fixed production owner descriptor. The HTTP path covers original verifier-pending work, fresh login/control, separate attachment and verifier approvals, deliberately lost durable-completion response, observational terminal reconciliation, old-control refusal and revoked-login refusal. One original model and one verifier gateway execution were observed; terminal retries did not replay either.
